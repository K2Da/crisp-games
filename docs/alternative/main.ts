...
let players: { position: Vector; angle: number }[] = [];
let isPressing: boolean;
let arrowAngle: number;
let arrowDirection: boolean;
let deathCube: { position: Vector; angle: number };
let pointCubes: {
  position: Vector;
  angle: number;
  size: number;
...
function update() {
  ...
  if (input.isPressed) {
    ...
    if (arrowAngle <= 0) arrowDirection = true;
    if (arrowAngle > PI) arrowDirection = false;
  }
  ...
  remove(players, (p) => {
    ...
    if (collision.isColliding.rect.green) {
      pointCubes.forEach((c) => {
        if (
          Math.abs(c.position.x - p.position.x) < c.size &&
